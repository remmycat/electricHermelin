...
function TwitterClient() {
  ...
  self.update_client = function update_client(){
    self.client = new globals.twit({
      consumer_key:         conf.vars.consumer_key,
      consumer_secret:      conf.vars.consumer_secret,
      access_token:         conf.profiles[conf.current_name].preferences.access_token.oauth_token,
      access_token_secret:  conf.profiles[conf.current_name].preferences.access_token.oauth_token_secret
    });
  }
  ...
  self.update_status = function update_status(text, reply_to_id, on_success, on_error) {
    ...
    self.client.post('statuses/update', {
      ...
      if(err) on_error(response);
      else on_success(data);
    });
